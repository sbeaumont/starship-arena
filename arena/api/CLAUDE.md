...
4. **Open on purpose:** `/games`, `/{game}/overview`, `/ship-types`, `/manual`, `/time` and
   everything under `/valhalla` need no login. The scoreboard, the ship stats, the rules, when a
   game processes and every game that is over are public; positions and orders in a game still
   being played are not.
